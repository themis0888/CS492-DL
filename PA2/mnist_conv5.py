...
def custom_model_fn(features, labels, mode):
    ...
    if depth > 3:
        conv = tf.layers.conv2d(
          inputs=conv, filters=32, kernel_size=[window, window],
    ...
    conv = tf.layers.conv2d(
      inputs=pool1, filters=64, kernel_size=[window, window], 
      padding="same", activation=tf.nn.relu)
    if depth > 3:
        conv = tf.layers.conv2d(
          inputs=conv, filters=64, kernel_size=[window, window],
          padding="same", activation=tf.nn.relu)
    if depth > 5:
        conv = tf.layers.conv2d(
          inputs=conv, filters=64, kernel_size=[window, window],
    ...
    pool2 = tf.layers.max_pooling2d(inputs=conv, pool_size=[2, 2], strides=2)
...
if __name__ == '__main__':
    ...
    ## ----------------------------------------- ##
    # window' + str(window) +'_
    np.save('20120888_network_nodropout_'+ str(depth) + '.npy', result)
